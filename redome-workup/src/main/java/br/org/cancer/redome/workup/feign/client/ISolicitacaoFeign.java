...
public interface ISolicitacaoFeign {
	...
	@PutMapping(value = "api/solicitacoes/{id}/atualizarsolicitacaoposcoleta")
	public SolicitacaoWorkupDTO atualizarSolicitacaoPosColeta(@PathVariable(required = true) Long id,
			@RequestBody(required = true) Long posColeta);
	
	
	@PutMapping(value = "api/solicitacoes/{id}/atualizarsolicitacaocontagemcelula")
	public SolicitacaoWorkupDTO atualizarSolicitacaoContagemCelula(@PathVariable(required = true) Long id);
