...
public class AppUtil {
	...
	/**
	 * Formata o matchClass do wmda - exemplo: "6/6/X"  ->  “06/06”
	 * 
	 * @param matchClass string matchClass vinda do wmda
	 * @return string formatada.
	 */
	@SuppressWarnings("null")
	public static String formatarMatchGrade(String matchClass) {
		if(matchClass == null) {
			return null;
		}
		String[] lista = matchClass.split("/");
		if(matchClass.length() < 7) {
			String n1 = lista[0]; String n2 = lista[1];
			if(lista[0].length() == 1) {
				n1 = "0" + lista[0];
			}
			if(lista[1].length() == 1) {
	...
	/**
	 * Formata o nmdpGrade do wmda - exemplo: "PP-P-" -> A=P, B=P, C=-  
	 * 
	 * @param nmdpGrade string nmdpGrade vinda do wmda
	 * @return string formatada.
	 */
	public static Map<String, String> formatarQualificacao(String nmdpGrade) {
		if(nmdpGrade == null) {
			return null;
		}
		Map<String,String> qualificacoes = new HashMap<>();
		String[] qualVal = nmdpGrade.trim().split("");
		String[] locus = {"A","B","C","DRB1","DQB1"};
		
		for (int i = 0; i < qualVal.length; i++) {				
			qualificacoes.put(locus[i], qualVal[i]); 
		}
	 ...
	 * @return string formatada.
	 */
	public static String formatarHlaComCaracteresX(String alelo) {
		if(alelo == null) {
			return null;
		}
		if(alelo.endsWith("XX")) {
			return alelo.substring(0, alelo.indexOf(":"));
		}
		return alelo;
	}

	/**
	 * Formata o primeiro alelo caso seja nulo (-) 
	 * 
	 * @param alelo1 string alelo vindo do wmda
	 * @param alele2 string alelo vindo do wmda
	 * @return string formatada.
	 */
	public static String normalizadoAlelo1(String alelo1, String alelo2) {
		return alelo1 == null && alelo2 != null ? BLANK : formatarHlaComCaracteresX(alelo1); 
	}

	/**
	 * Formata o segundo alelo caso seja nulo (-) 
	 * 
	 * @param alelo1 string alelo vindo do wmda
	 * @param alele2 string alelo vindo do wmda
	 * @return string formatada.
	 */
	public static String normalizadoAlelo2(String alelo2, String alelo1) {
		return alelo2 == null && alelo1 != null ? BLANK : formatarHlaComCaracteresX(alelo2); 
	}
