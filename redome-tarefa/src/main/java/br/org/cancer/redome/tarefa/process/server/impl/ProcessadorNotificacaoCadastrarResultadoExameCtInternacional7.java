...
@Service
@Transactional
public class ProcessadorNotificacaoCadastrarResultadoExameCtInternacional7 implements IProcessadorTarefa {
		...
		if (tempoExecutando >= tempoExecucao) {
			...
			if (tarefaAVerificar.getStatus().equals(StatusTarefa.ABERTA)) {				
				...
				if (tarefasNotificacao.size() < quantidade) {
					...
					exameFeign.notificacarCadastroResultadoExameCtInternacional7(tarefaAVerificar.getId());
				}
			}
