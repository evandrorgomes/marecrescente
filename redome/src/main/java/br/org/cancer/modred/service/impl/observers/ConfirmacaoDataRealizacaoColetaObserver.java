 ...
 */
@Component
public class ConfirmacaoDataRealizacaoColetaObserver extends EntityObserver<Disponibilidade> {
	...
	public boolean condition(Disponibilidade disponibilidade) {
		return disponibilidade.getPedidoColeta() != null && disponibilidade.getDataAceite() == null;
	}

	@SuppressWarnings("rawtypes")
	@Override
	public void process(Disponibilidade disponibilidade) {
		final Match match = disponibilidade.getPedidoColeta().getSolicitacao().getMatch();
		...
		CategoriasNotificacao.PRESCRICAO.getConfiguracao()
			...
			.comDescricao(
				AppUtil.getMensagem(messageSource, "pedidocoleta.solicitar.confirmacao.data.realizacao.centrotransplante.notificacao", 
