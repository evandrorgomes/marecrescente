 ...
 * é reprovada (não prossegue).
 * 
 */
@Component
public class AvaliacaoPedidoColetaReprovadoObserver extends EntityObserver<AvaliacaoPedidoColeta> {
	...
	public boolean condition(AvaliacaoPedidoColeta avaliacao) {
		return avaliacao.getPedidoProssegue() != null 
					&& Boolean.FALSE.equals(avaliacao.getPedidoProssegue());
	}
	...
	public void process(AvaliacaoPedidoColeta avaliacao) {
		...
		final String textoNotificacao =
				AppUtil.getMensagem(messageSource, 
						"notificacao.medico.resultado_avaliacao_pedido_coleta_reprovado", doador.getIdentificacao().toString(), rmr);
		...
		CategoriasNotificacao.AVALIACAO_PEDIDO_COLETA.getConfiguracao()
			...
			.comDescricao(AppUtil.getMensagem(messageSource, 
					"notificacao.analista_workup.resultado_avaliacao_pedido_coleta_reprovado"))
			.apply();
	}
