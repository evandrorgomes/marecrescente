 ...
 */
@Repository
public interface IUsuarioRepository extends IRepository<Usuario, Long> {
	...
	@Query("select u from Usuario u inner join u.perfis p where p.id = :perfil order by u.nome ")
	List<Usuario> listarUsuariosPorPerfil(@Param("perfil") Long idPerfil);
	...
	@Query("select u from Usuario u inner join u.laboratorio l inner join u.perfis p where l.id = :laboratorio and p.id = :perfil ")
	List<Usuario> listarUsuariosPorLaboratorioEPerfil(@Param("laboratorio") Long idLaboratorio, @Param("perfil") Long idPerfil);
