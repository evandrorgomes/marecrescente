...
public class H2CordaoInternacionalControllerTest extends BaseConfigurationTest {
	...
	public void setupBefore() {
		...
		ReflectionTestUtils.setField(cordaoInternacionalService, "executarProcedureMatchService", executarProcedureMatchService);
	 ...
	 */
	@Test
	public void deveSavarCordaoInternacionalAssociadoAoPaciente() throws Exception {
		...
		Long rmr = H2BasePaciente.criarPacienteComExameAprovadoEAprovacaoInicial(mockMvc, pacienteService, avaliacaoService, 
				...
				.comONome("TESTE DE PACIENTE COM CORDAO INTERNACIONAL")
				.comOCpf("13786240094")
				.comOCns("224464372640001"));
				
		DoadorCordaoInternacionalDTO doador = criarCordaoInternacional(rmr, "784784094", null);
	...
	public void deveAtualizarDoadorInternacional() throws Exception {
		...
		Long rmr = H2BasePaciente.criarPacienteComExameAprovadoEAprovacaoInicial(mockMvc, pacienteService, avaliacaoService, 
				examePacienteService, genotipoService, logEvolucaoService, new PacienteDto()
				.comADataNascimento(LocalDate.now().minusYears(25))
				.comONome("TESTE DE PACENITE COM CORDÃO internacional")
				.comOCpf("73477593073")
				.comOCns("172563864720000"));
				
		DoadorCordaoInternacionalDTO doador = criarCordaoInternacional(rmr, "ikdfjkf904", null);
		MockMultipartFile doadorJson = new MockMultipartFile("doadorCordaoInternacional", "", "application/json",BaseConfigurationTest.getObjectMapper().writeValueAsBytes(doador));		
		...
		Long idDoador = cordaoInternacionalService.obterIdentifiadorCordaoInternacionalPorIdDoRegistro("ikdfjkf904");
		
		Assert.assertNotNull(idDoador);
		Assert.assertNotEquals(Long.parseLong("0"), idDoador.longValue());
		
		doador = criarCordaoInternacional(null, "ikdfjkf904", null);
		doador.setVolume(new BigDecimal(30));
		doador.setSexo("M");
	...
	private DoadorCordaoInternacionalDTO criarCordaoInternacional(Long rmr, String idRegistro, String grid) {
